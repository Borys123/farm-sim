...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAIControl : MonoBehaviour
    {
        ...
        [Header("Enter Exit Vehicle Settings")]
        //|||||||||||||||||||||||||\\
        //|||||||||||||||||||||||||\\
        //|||||||||||||||||||||||||\\
        ...
        private void OnTriggerStay(Collider other)
        {
            if(other.gameObject.name == "Character")
            {
                if(Input.GetKeyDown(KeyCode.E) ) { 
                    NextCamera();
                    Destroy(gameObject);
                    Instantiate (carObjects, spawnPoint.position, spawnPoint.rotation);
                    Debug.Log("AI Car");
        ...
        {
            int nextactiveCamera = m_CurrentActiveCamera + 1 >= driverManager.cameras.Length ? 0 : m_CurrentActiveCamera + 1;
            for (int i = 0; i < driverManager.cameras.Length; i++)
            {
                driverManager.cameras[i].SetActive(i == nextactiveCamera);
            }
            m_CurrentActiveCamera = nextactiveCamera;
                    
            int nextactivecharacter = m_CurrentActiveCharacter + 1 >= driverManager.characters.Length ? 0 : m_CurrentActiveCharacter + 1;
            for (int i = 0; i < driverManager.characters.Length; i++)
            {
                driverManager.characters[i].SetActive(i == nextactivecharacter);
            }
